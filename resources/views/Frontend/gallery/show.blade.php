...
<section class="relative min-h-[50vh] flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-20 text-center text-white px-4 max-w-4xl mx-auto" data-aos="fade-up" data-aos-duration="1000">
        <div class="flex items-center justify-center gap-2 mb-4" data-aos="fade-up" data-aos-delay="100">
            @if($gallery->featured)
            ...
            @endif
            @if($gallery->category)
                <span class="bg-gradient-to-r from-teal-500 to-cyan-500 text-white px-4 py-2 rounded-full text-sm font-semibold">
                    {{ ucfirst($gallery->category) }}
                </span>
            @endif
        </div>
        
        <h1 class="text-3xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-white to-teal-100 bg-clip-text text-transparent">
            {{ $gallery->title }}
        </h1>
        
        <div class="flex items-center justify-center gap-6 text-teal-100 mb-6" data-aos="fade-up" data-aos-delay="200">
            ...
            @if($gallery->trip_date)
                <div class="flex items-center gap-2">
                    ...
                    <span>{{ $gallery->trip_date->format('d F Y') }}</span>
                </div>
            @endif
            ...
            <div class="flex items-center gap-2">
                ...
                <span id="view-count">{{ number_format($gallery->views) }} views</span>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center gap-4" data-aos="fade-up" data-aos-delay="300">
            <button id="like-btn" data-gallery-id="{{ $gallery->id }}" 
                    class="flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-6 py-3 rounded-lg font-medium transition-all duration-300 transform hover:scale-105">
                <svg id="like-icon" class="w-5 h-5 {{ $gallery->isLikedByUser() ? 'fill-red-500 text-red-500' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                </svg>
                <span id="like-count">{{ number_format($gallery->likes) }}</span>
            </button>
            
            <button onclick="shareGallery()" class="flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-6 py-3 rounded-lg font-medium transition-all duration-300 transform hover:scale-105">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('gallery') }}" class="flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-6 py-3 rounded-lg font-medium transition-all duration-300 transform hover:scale-105">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="container mx-auto px-4">
            ...
            <div class="max-w-6xl mx-auto mb-12">
                <div class="relative rounded-2xl overflow-hidden shadow-2xl" data-aos="fade-up">
                    ...
                    @if(count($gallery->images) > 1)
                        <button id="prev-btn" class="absolute left-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-all duration-300">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </button>
                        <button id="next-btn" class="absolute right-4 top-1/2 transform -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-all duration-300">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Image Counter -->
                    @if(count($gallery->images) > 1)
                        <div class="absolute bottom-4 right-4 bg-black/70 text-white px-4 py-2 rounded-lg">
                    ...
                    <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4">
                        @foreach($gallery->images as $index => $image)
                            <div class="thumbnail cursor-pointer rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105 {{ $index === 0 ? 'ring-2 ring-teal-500' : '' }}" 
                                ...
                                <img src="{{ Storage::url($image) }}" alt="Gallery {{ $index + 1 }}" class="w-full h-20 object-cover">
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @if($gallery->description)
            <div class="max-w-4xl mx-auto" data-aos="fade-up" data-aos-delay="300">
                <div class="bg-gradient-to-br from-gray-50 to-slate-100 rounded-2xl p-8">
                    <h3 class="text-2xl font-bold text-gray-800 mb-4">Tentang Gallery Ini</h3>
...
<section class="py-16 bg-gradient-to-br from-gray-50 to-slate-100">
    <div class="container mx-auto px-4">
        <div class="text-center mb-12" data-aos="fade-up">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Gallery Terkait</h2>
            <p class="text-lg text-gray-600">Jelajahi gallery lainnya yang mungkin Anda sukai</p>
        </div>
                ...
                <div class="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center text-gray-600 mb-3">
                            ...
                            <span class="text-sm">{{ $related->destination }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('gallery.show', $related->slug) }}" 
                               class="bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white px-3 py-1 rounded-lg text-xs font-medium transition-all duration-300">
                                Lihat
                            </a>
                        </div>
                    </div>
